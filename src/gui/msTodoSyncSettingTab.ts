...
export const DEFAULT_SETTINGS: IMsTodoSyncSettings = {
    ...
    microsoftToDoApplication_RedirectUriBase: '',
};
...
export class MsTodoSyncSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        this.addTextSetting(
            ...
            async (value) => {
                ...
                this.userNotice.showMessage('Please restart Obsidian for the new Client ID to take effect.', 10000);
            },
        );

        // Advanced settings (only show if hacking mode enabled)
    ...
    async hide() {
        ...
        if (!listName) {
            // No list name set - that's fine, user can select per-task
            return;
        }

        // Only look up/create if listId is not set or listName changed
        if (this.settings.todoListSync.listId === undefined) {
            // First try to find existing list
            let listId = await this.plugin.todoApi.getListIdByName(listName);

            // Only create if it doesn't exist
            if (!listId && this.settings.todo_CreateToDoListIfMissing) {
            ...
            if (listId) {
                ...
                await this.plugin.saveSettings();
            } else if (this.settings.todo_CreateToDoListIfMissing) {
